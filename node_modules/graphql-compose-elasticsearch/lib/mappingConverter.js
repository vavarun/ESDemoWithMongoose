...
function inputPropertiesToGraphQLTypes(prop, fieldName) {
  ...
  // mapping
  if (prop.properties && (0, _graphqlCompose.isObject)(prop.properties)) {
    Object.keys(prop.properties).forEach(function (subFieldName) {
      inputPropertiesToGraphQLTypes(
      // $FlowFixMe
      prop.properties[subFieldName], [fieldName, subFieldName].filter(function (o) {
        return !!o;
      }).join('__'), result);
    });
    return result;
  ...
  // object type with subfields
  if (prop.fields && (0, _graphqlCompose.isObject)(prop.fields)) {
    Object.keys(prop.fields).forEach(function (subFieldName) {
      inputPropertiesToGraphQLTypes(
      // $FlowFixMe
      prop.fields[subFieldName], [fieldName, subFieldName].filter(function (o) {
        return !!o;
      }).join('__'), result);
    });
  }
