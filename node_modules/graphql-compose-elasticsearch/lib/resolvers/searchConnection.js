            ...
            case 24:
              ...
              edges = list.map(function (node) {
                ...
                if (cursorMap.has(cursor)) {
                  throw new Error('Argument `sort` should be more complex. `cursor` are constructed ' + 'according to the sort fields. Detected that two records have ' + `the same cursors '${cursor}' with data '${unbase64(cursor)}'. ` + 'You should add more `sort` fields, which provide unique data ' + 'for all cursors in the result set (eg. `id` field).');
                }
